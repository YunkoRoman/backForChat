...
- [ ] 12.1 Write `apps/api/Dockerfile` (multi-stage build); verify `docker build` succeeds and the image starts against a local `.env`
- [ ] 12.2 Write root `docker-compose.yml` with `mongo`, `rabbitmq` (management UI on `:15672`), and `api` services, healthchecks, and named volumes; verify `docker compose up` brings up a working API reachable on its published port
- [ ] 12.3 Document local dev setup (`.env` from `.env.example`, `docker compose up`, seed/test commands) in `apps/api/README.md`
